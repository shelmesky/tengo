...
import (
	...
	"github.com/shelmesky/tengo/compiler/scanner"
...
/*
parseSimpleStmt函数名称的意思是处理简单语句, 简单语句包括:
1. 赋值语句
2. for(init; cond; post) 中的语句
3. for(x in xxx) 中的x in xxx
4. x++
这些语句有的包含操作符, 有的未包含, 但都由此函数处理.
由于parseExprList中调用的是解析i表达式, 但是表达式又允许字面值的存在,
所以甚至字面值单独出现在一行, 语法上也是允许的, 但没有任何意义.
*/
func (p *Parser) parseSimpleStmt(forIn bool) ast.Stmt {
	...
	/*
		这里如果从传统的角度理解会出现难以理解的情况, 因为按照正常理解这里应该是解析语句中的左值.
		但表达式又是右值, 所以会出现上面提到的: 甚至字面值单独出现在一行语法上也允许.
		实际上就是因为调用了解析表达式的方法去解析左值, 才会出现这个现象.
		结论就是这里实际上模糊了语句和表达式的概念, 统一由此函数解析.
	*/
	x := p.parseExprList()
